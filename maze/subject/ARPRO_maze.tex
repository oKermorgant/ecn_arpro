...
\begin{itemize}
 \item \texttt{void Node::children(vector<Node*> \&)}: updates the argument to pointers to the children (or neighboors) of the considered
 element
 \item \texttt{int distToParent()}: returns the distance to the node that generated this one
 \item \texttt{bool is(Node*)}: returns true if the passed argument is actually the same point
